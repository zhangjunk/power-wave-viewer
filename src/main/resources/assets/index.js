...
async function loadData() {
    ...
    if (!beginDate?.trim() !== !endDate?.trim()) {
        alert("开始日期和结束日期必须都选或都为空！");
        return;
    }
    if (beginDate?.trim() && endDate?.trim() && !validDateRange(beginDate, endDate)) {
 ...
 * @returns {Boolean}
 */
function validDateRange(beginDate, endDate) {
    // 2. 转时间戳
    const beginStamp = new Date(beginDate).getTime();
    const endStamp = new Date(endDate).getTime();
    // 3. 合法性校验：如果是非法日期，getTime()返回NaN
    if (isNaN(beginStamp) || isNaN(endStamp)) {
        return false;
    }
    // 4. 核心判断：结束时间戳 > 开始时间戳
...
async function getGraphData(fileName) {
    // 清空之前的错误/加载状态，高亮选中的li
    highlightSelectedLi(fileName);

    //获取PRPD图谱数据：相位分辨局部放电图谱
    await getPrpdData(fileName);
    //获取PRPS图谱数据：相位分辨脉冲序列图谱
    await getPrpsData(fileName);
    if (stationName=='220kV龙河站'){
        //获取SYBX图谱数据：时域波形
        await getSybxData(fileName);
    }
}
...
function initPrpsChart(ajaxResult){
    const minWave = ajaxResult.data.minWave;
    const maxWave = ajaxResult.data.maxWave;
    if (minWave<0){
        // ✅ ✅ ✅ 核心步骤1：数据正向映射 【一行代码搞定】
        const renderData = ajaxResult.data.chartData.map(item => {
            const [x, y, val] = item;
            return [x, y, val -ajaxResult.data.minWave]; // 原始[-80,0] → 映射后[0,80]
        });
        ajaxResult.data.chartData = renderData;
    }

    //const min = ajaxResult.data.minWave<0?ajaxResult.data.maxWave:ajaxResult.data.minWave;
    //const max = ajaxResult.data.maxWave==0?-ajaxResult.data.minWave:ajaxResult.data.maxWave;
    const min = minWave<0?maxWave:Math.floor(minWave/10)*10;
    const max = maxWave==0?-minWave:Math.floor(maxWave/10)*10+10;
    ...
    const option = {
        ...
        title: [
            {
                text: 'PRPS图谱',
                left: 'left',
                //subtext: '工频周期(X) × 相位角度(Y) × 脉冲幅值(Z)',
                textStyle: { color: '#1E88E5', fontSize: 12 }
            }
            ,{
                text: '工频周期(X) × 相位角度(Y) × 脉冲幅值(Z)',
                right: '10px', // 距离右侧10px
        ...
        tooltip: {
            ...
            formatter: function (params) {
                const realAmp = params.value[2] -Math.abs(ajaxResult.data.minWave); //前端tooltip取负，还原真实的 -xx dBm 幅值
                return `
                工频周期：第 ${params.value[0]+1} 个<br>
                相位角度：${params.value[1].toFixed(2)}°<br>
                脉冲幅值：${realAmp.toFixed(2)}
            `;
            }
        },
        // 颜色比例尺（对应强度）
        visualMap: {
            ...
            top: 'center',
            itemWidth: 10,
            text: minWave < 0 ? ['脉冲幅值(0)', '脉冲幅值(-80)'] : ['HIGH', 'LOW'], // 上下文字标注
            calculable: true,
            inRange: {
                color: ['#003399', '#99CCFF', '#FFFF99', '#FFCC00', '#FF4D00']
                //color: [ '#FF4D00', '#FFCC00', '#FFFF99', '#99CCFF','#003399']
            },
            // ========== 以下3行是核心！只隐藏数值，其他都保留 ==========
            /**showLabel: false,    // 隐藏 min/max 的数值文字（0 和 80 彻底消失）
            axisLine: { show: false }, // 隐藏颜色条的数值轴线
            tickLine: { show: false }, // 隐藏颜色条的刻度线**/
        ...
        grid3D: {
            ...
            // ✅ 3D视角最佳角度（电力PRPS专用，无需修改）
            viewControl: {
                /**autoRotate: true,  // 开启自动旋转，展示3D全貌
                autoRotateSpeed: 3,// 旋转速度，适中
                rotateSensitivity: 1, // 鼠标拖拽旋转灵敏度
                zoomSensitivity: 1,   // 滚轮缩放灵敏度
                panSensitivity: 1,    // 鼠标平移灵敏度
                distance: 200,        // 视角距离，最佳展示距离
                alpha: 45,            // 上下倾斜角度
                beta: 30,              // 左右旋转角度
            ...
            // 光照效果：提升3D立体感，柔和不刺眼
            light: {
                main: {color: '#fff', intensity: 1.2},
                ambient: {color: '#fff', intensity: 0.8},
                ambientCubemap: {texture: '', exposure: 1, diffuseIntensity: 1}
            }
        },
        xAxis3D: {
            name: '工频周期数',
            ...
            max: ajaxResult.data.powerCycleCount-1,
            //offset:5
            axisLine: {
                lineStyle: {
                    color: '#1E88E5',
        ...
        zAxis3D: {
            ...
            min: min,// 实际渲染最小值 0 (对应视觉-80)
            max: max,// 实际渲染最大值 80 (对应视觉0)
            nameTextStyle: { fontSize: 14 },
            axisLabel: {
                fontSize: 12,
                // ✅ 核心格式化：把[0,80]显示成[-80,0]，视觉完美匹配
                //formatter: (value) => value - max
                formatter: (value) => minWave < 0 ? (value - max) : value
            },
            splitLine: { show: true },
            splitNumber: 8, // 均分8段，刻度完美显示-80/-70/-60...0
            //interval:10,
            axisLine: {
                lineStyle: {
                    color: '#1E88E5',
        ...
        series: [
            {
                ...
                emphasis: {
                    itemStyle: {color: '#FF5722'}, // 橙色高亮，醒目
                    label: {
                        show: false
                    }
                }
        ...
        // ✅ 无数据兜底提示：和你之前的配置完全一致，无缝复用
        graphic: ajaxResult.data.chartData.length === 0 ? [{
            type: 'text',
            left: 'center',
            top: 'center',
            text: '当前无有效局放脉冲',
    ...
    try {
        ...
        const option = {
            ...
            visualMap: {
                ...
                right: '2px',
                top: 'center',
                orient: 'vertical',    // 你的垂直样式不变
                align: 'right',        // 文本在右侧 不变
                itemWidth: 10,
                ...
                // 比例尺样式
                //text: ['脉冲次数(高)', '脉冲次数(低)'],
                text: minWave < 0 ? ['脉冲次数(高)', '脉冲次数(低)'] : ['HIGH', 'LOW'], // 上下文字标注
...
function initTimeDomainChart(chartId, pdData) {
    ...
    const option = {
        ...
        title: [
            ...
            // 第二个标题：右侧幅值文本
            {
                text: `幅值极值：最小值 ${minWave.toFixed(2)} | 最大值 ${maxWave.toFixed(2)}`,
                right: '10px', // 距离右侧10px
                //top: '5px',
                textStyle: { color: '#666', fontSize: 12 }
            }
        ...
        tooltip: {
            ...
            formatter: function(params) {
                ...
                return `时间：${time} ms<br/>相位：${phase.toFixed(1)}°<br/>幅值：${amp}`;
            }
        },
        series: [
            {
        ...
        // ✅ 无数据兜底提示：和你之前的配置完全一致，无缝复用
        graphic: pdData.chartData.length === 0 ? [{
            type: 'text',
            left: 'center',
            top: 'center',
            text: '当前无有效局放脉冲',
 ...
 * @returns {Array} ECharts所需的[时间(ms), 幅值()]格式数据
 */
